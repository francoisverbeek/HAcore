...
from .api import BASE_API, BASE_HEADERS
...
class Plant:
    ...
    async def _get_total_grid(self, coordinator, headers):
        if self.inverter_sn is None:
            _LOGGER.INFO("Inverter SN not set, skipping")
            return
        res = await coordinator.session.get(
    ...
    async def _get_total_pv(self, coordinator, headers):
        ...
        returned = await res.json()
        self.acc_pv = sum(
            decimal.Decimal(i["value"]) for i in returned["data"]["infos"][0]["records"]
        )

    async def _get_total_load(self, coordinator, headers):
        if self.inverter_sn is None:
            _LOGGER.debug("Inverter SN not set, skipping")
            return
        res = await coordinator.session.get(
